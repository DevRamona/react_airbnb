...
const CommunityForum = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      const fakePosts = [
        { id: 1, title: 'Best fertilizer for maize', author: 'JohnF', content: 'I\'ve been trying different fertilizers for my maize crop. Any recommendations?', replies: [], timestamp: new Date(Date.now() - 86400000).toISOString() },
        { id: 2, title: 'Dealing with tomato blight', author: 'MariaK', content: 'My tomato plants are showing signs of blight. How can I treat this?', replies: [], timestamp: new Date(Date.now() - 172800000).toISOString() },
      ];
      setPosts(fakePosts);
    };
    ...
    if (selectedPost && newReply.trim() !== '') {
      const updatedPosts = posts.map(post => 
        post.id === selectedPost.id 
          ? { 
              ...post, 
              replies: [...post.replies, { id: post.replies.length + 1, content: newReply, author: 'CurrentUser', timestamp: new Date().toISOString() }]
            }
          : post
      );
      ...
      setSelectedPost({
        ...selectedPost,
        replies: [...selectedPost.replies, { id: selectedPost.replies.length + 1, content: newReply, author: 'CurrentUser', timestamp: new Date().toISOString() }]
      });
      setNewReply('');
    }
  };
